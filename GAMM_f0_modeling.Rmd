---
title: 'Modeling intonational tunes with Generalized Additive Mixed Models: A Brief
  Introduction'
...
If you want to get more hands on, you can access the corresponding data and RMarkdown document to experiment with the code, and fit your own models with the data. 

This is intended to be a mostly conceptual introduction- it glosses over more than a few details. 

* For much more in-depth descriptions of GAM(M)s see, e.g. @soskuthy2021evaluating, @wood2017generalized

* The document includes notes for <span style="color: purple;">further reading</span> where relevant. 

## The data  

The data come from an imitative speech paradigm in which speakers heard model utterance such as "Her name is Marilyn", with f0 resynthesized to create various tunes. Speakers reproduced what they heard on a new sentence such as "They honored Melanie", effectively transposing the tune. 
...
Let's look at two types of basis functions in different number fit to the HLH trajectory. 
...
In practice, setting a very high number of knots is not a good idea because its computationally intensive, and can sometimes lead to over-fitting. 
...
There is no hard and fast rule for choosing the right number of knots, and the number you select should be informed by how "wiggly" you expect your data to be. 
 ...
 In my experience 10 knots is a good amount for modeling trajectories of the sort we see in intonational tunes. The package ```mgcv``` comes with useful ways to check that the number of knots you are setting is appropriate.
...
In the preceding toy examples, we are fitting a single trajectory, that is, one time series of f0. However, everything we just saw applies when we scale up to modeling, say, all of the LHH contours in the data set, as in:
...
Let's look at the fit in the same way as with the single-trjacetory models: 

```{r,echo=FALSE,message=FALSE,fig.height = 3.5, fig.width = 9.5}
...
ggplot(data[data$tune=="LHH",],aes(x=prop_dur,y=erbc))+
    geom_line(alpha=0.2,size=1.5,aes(group=speaker_trial))+ 
  geom_line(data=model_matrix_tp_10_LHH[model_matrix_tp_10_LHH$index!="j",],aes(x=prop_dur,y=coeff_basis,color=index),size=1)+
...
## Comparing tunes, adding random effects, and error models

So far we've just fit GAMMs to a single tune's trajectory, allowing us to characterize its shape. 

What if we want to compare tunes to one another? The remainder of this tutorial will walk through this, comparing just two tunes: HHH and HHL, the "high rising" tunes. 
...
By plotting the residuals like before we can see that the model fit is overall pretty similar to the non-AR1 variant of the model 
...
There are various ways to do significance testing in GAMMs, and the appropriate one may very depending on your question. These include: 

* Inspecting coefficients that model differences explicitly (not possible with the way we fit our model)
* Model comparison 
* Visual inspection of fits, differences and CI. 
...
Here are some things you can do to extend what was covered above. 
...
* Compare autocorrelation from this new model that you make to one which includes and AR1 error model. 
